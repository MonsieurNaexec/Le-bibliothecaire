...
class Notification extends Command {
  ...
  constructor(commandManager) {
    ...
    this.addOption(
      "role",
      "Role à mentionner lors des annonces",
      "ROLE",
      false
    );
  }
  ...
  execute(interaction) {
    ...
    interaction
      .reply({
        content: ":clock2: Enregistrement du salon d'annnonce...",
        ephemeral: true,
      })
      .then(() => {
        googleAPI
          ...
          .then((e) => {
            interaction.editReply(
              `:white_check_mark: Ce salon recevra les annonces pour la catégorie \`${interaction.options.getString(
                "categorie"
              )}\``
            );
          })
